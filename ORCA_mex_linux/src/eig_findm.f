...
c
      ...
      if(iifail .eq. 1) then
cyt         print *,'iifail=1 in eig_final: nmode = ',nmode
cyt         call stop_run
         return
      endif
c
...
c: OK if we found a branch line mode very close to branch point.
            ...
            nc_ref=nc_ref + 1
            if(nc_ref .gt. 1) then
cyt               if(iidiag .ge. 1) print *,'Informative message: ',
cyt     .            'XKREF island mode not resolved.'
               iifail=2
               return
            endif
cyt            if(iidiag .ge. 1) print *,'FALSE MODE AT XKREF IGNORED: ',
cyt     .         dreal(xkref)/kw0,dimag(xkref)*8685.9
...
cyt         if(iidiag .ge. 1) print *,'iicirc incr: nmode = ',nmode,
cyt     .      iicirc,dreal(k)/kw0,dimag(k)*8685.9d0,del_r1r2
...
cyt               if(iimt .ne. 0) call mode_traj(k0,rr0,0)
               goto 999
...
cyt25             if(iimt .eq. 1) call mode_traj(k_sdp,rr0,0)
cyt               kstart=k_sdp
25               kstart=k_sdp
...
cyt                     print *,'traj_sdp unsuccessful '
cyt                     print *,'iicirc>4: Tell EKW ',nmode
                     nmode=nmode - 1
                     return
                  endif
               endif
...
cyt         if(iidiag .ge. 1) print *,'Island mode successfully '//
cyt     .      'bypassed: ',nmode,iicirc,dreal(k)/kw0,dimag(k)*8685.9d0
         iicirc=0
         emagmax0=0.1d0
         ephmax0=0.19635d0
      endif
...
cyt         if(iimt .ne. 0) call mode_traj(k,r1r2,-2)
         if(branch .ne. (0.d0,0.d0)) then
c: Mode in lower half plane because we are on wrong sheet:
            if(iidiag .ge. 1) then
cyt               print *,'Informative message: MODE FOUND IN LOWER HP ',
cyt     .            'ON BAD SHEET: ',nmode,k,k/kw0
cyt               print *,'   MODE IGNORED.  ATTEMPTING TO PICK UP PATH',
cyt     .            ' ALONG BRANCH CUT ...'
            endif
            call fix_path(k0,rr0,1,0)
            if(iidone .eq. 1 .or. iifail .eq. 1) return
         else
c: Continue on trajectory, but skip mode found in lower half plane:
            if(iidiag .ge. 1) then
cyt               print *,'Informative message: MODE FOUND IN LOWER HP ',
cyt     .            'ON OK SHEET: ',nmode,k,k/kw0
cyt               print *,'  MODE IGNORED, SEARCH CONTINUING ON PATH ...'
            endif
            k0=k
            rr0(1,4)=r1r2(1,4)
            rr0(2,4)=r1r2(2,4)
...
c
cyt      if(iimt .ne. 0) call mode_traj(k,r1r2,1)
c
...
cyt         if(iimt .ne. 0) call mode_traj(k,r1r2,-2)
         ...
         ntry2=ntry2 + 1
         if(ntry2 .gt. 45) then
cyt            print *,'ntry2 trouble in eig_final: ',dreal(k)/kw0,
cyt     .         dimag(k)*8685.9,delk
            if(ntry2 .gt. 50) then
               iifail=1
               return
            endif
      ...
      ntry=ntry + 1
      if(ntry .gt. 45) then
cyt         print *,'ntry trouble in eig_final: ',dreal(k)/kw0,
cyt     .      dimag(k)*8685.9,delk
         if(ntry .gt. 50) then
            iifail=1
            return
...
c
      ...
      integer*4 j,jj
c
cyt      print *,'Unable to follow |R1R2|=1 contour.  Giving up.'
cyt      print *,'Will continue with modes already found... '
      if(iidiag .ge. 1) then
cyt         print *,'PROFILE TO FOLLOW. nmode = ',nmode
         do j=1,nlay
cyt            print *,j,1,h(j),(geo(1,jj,j),jj=1,5)
cyt            print *,j,2,h(j),(geo(2,jj,j),jj=1,5)
         enddo
      endif
      nmode=nmode - 1
c
...
c
      ...
      if(delk_re .gt. 0.d0) then
         if(nfix .eq. 0) then
            if(iidiag .ge. 2) then
cyt         print *,'Informative message: MODE ON BAD SHEET FOUND: ',
cyt     .      k/kw0,branch/kw0
cyt         print *,'   MODE IGNORED.  ATTEMPTING TO PICK UP PATH',
cyt     .      ' ALONG BRANCH CUT ...'
            endif
cyt            if(iimt .ne. 0) call mode_traj(k,r1r2,-3)
            call fix_path(k0,rr0,1,0)
            if(iidone .eq. 1 .or. iifail .eq. 1) return
            nfix=nfix + 1
            ii999=1
         else
cyt      print *,'Mode on wrong sheet near branch point kept',nmode,
cyt     .   dreal(k)/kw0,dimag(k)*8685.9
         endif
      else
cyt         if(iidiag .ge. 1) print *,'OK(?) MODE ON -1 SHEET FOUND: ',
cyt     .      k/kw0,branch/kw0,iish
      endif
c
      return
      end
