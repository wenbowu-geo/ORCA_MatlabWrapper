...
c yt               write(2,209)
c yt209            format('   BLUG layer subdivided: ')
               do i=nlay-nadd,nlay
c yt                  write(2,208) h(i),((geo(ji,ii,i),ji=1,2),ii=1,5)
c yt208               format(11(f10.4,1x))
               enddo
            endif
         endif
      enddo
...
c yt            print *,'Duplicate depth in SVP ignored: ',j,j+1,zsvp(j)
         endif
      enddo
      jobot=nlay
c
...
c: Temp to place ducts at user's request:
      if(iidiag .eq. -8) then
55       continue
c yt         print *,'Enter layer#, ii(1=top,2=bot) to place ref depth: '
         read(5,*) j,ii
         if(j .ne. 0) then
            c4=geo(ii,1,j)
c            call peak_enter(nduct,zdep,j,jlast,c4,c4,1)
            print *, 'call to peak_enter with 7 args commented out'
...
c yt         print *,'nduct=0 or cfmin bad: BUG - tell EKW ',
c yt     .      (csvp(j),j=1,nsvp)
         stop
      endif
c
c: Sort dz_duct:
...
c      return
c BD
c               open (unit = 51, file ='Err1.txt', action='write')
c               write (51,*) 'Stop in first loop'
c               close (51)
c
...
c: problems in rp_nomm when on negative sheets):
     ...
     .         isp(j+1) .eq. 1) then
c yt               print *,'ORCA CANNOT HANDLE ISOSPEED P-WAVE LAYERS '//
c yt     .   'ON BOTH SIDES OF NO-MISMATCH-IN-P S-S INTERFACE!! SORRY!'
c yt               print *,'Layer # ',j,j+1
c yt               print *,'Put in slight discontinuity or gradient.'
c               open (unit = 51, file ='Err1.txt', action='write')
c               write (51,*) 'Stop in first loop'
c               close (51)
               iibad=1
...
c yt               print *,'ORCA CANNOT HANDLE ISOSPEED S-WAVE LAYERS '//
c yt     .   'ON BOTH SIDES OF NO-MISMATCH-IN-S INTERFACE!! SORRY!'
c yt               print *,'Layer # ',j,j+1
c yt               print *,'Put in slight discontinuity or gradient.'
c               open(unit=52,file='Err2.txt',action='write')
c               write (52,*) 'Stop in second loop'
c               close (52)
               iibad=1
            endif
         endif
...
c BD
c
c BD THIS LINE IS FORCING STOP
      if(iibad .eq. 1) stop
c BD
c
c
...
c
      return
c yt 500   print *,'Error opening SVP file ',svp_file
500      stop
      end
ccc
...
c
      csmax=cp*cratlim
      if(cs .gt. csmax) then
c yt         print *,'WARNING: Positive compressibility requires ',
c yt     .      'cs/cp < sqrt(3/4).'
c yt         print *,'         Layer # ',jlay,'; cs,csmax = ', cs,csmax
c yt         write(2,100) jlay,cs,csmax
         cs=csmax
      endif
c yt100   format('WARNING: Positive compressibility requires ',
c yt     .      'cs/cp < sqrt(3/4).'/
c yt     .      '          Layer # = ',i3,'; cs,csmax = ',f7.1,2x,f7.1)
c
...
c yt         print *,'WARNING: Conservation of energy requires ',
c yt     .      'as/ap < (3/4)*(cp/cs)**3.'
c yt         print *,'         Layer # ',jlay,'; as,asmax = ',as,asmax
c yt         write(2,110) jlay,as,asmax
         as=safe_fac*asmax
      endif
c yt110   format('WARNING: Conservation of energy requires ',
c yt     .      'as/ap < (3/4)*(cp/cs)**3.'/
c yt     .      '          Layer # = ',i3,'; as,asmax = ',f9.4,2x,f9.4)
c
      return
      end
ccc
...
c
      if(nduct .ge. nd_next) then
         if(iidiag .ge. 1) then
c yt            print *,'Duct skipped (two in a row): ',jlay
         endif
         return
      endif
c
...
c yt         print *,'Duct skipped (no-mismatch lower h-space)'
         return
      elseif(jlay .eq. 2 .and. mm(1) .eq. 0 .and.
     .   isp(2) .eq. 1) then
c yt         print *,'Duct skipped (no-mismatch upper h-space)'
         return
      endif
      nduct=nduct + 1
      jduct(1,nduct)=jlay
...
c
      ...
      use gen_com
c     BD
c     BD jlay is regularly passed a real*8
c     BD
      integer*4 nd_next,jlay,jp,mm_dn
      real*8 c4,c_dn,dwid,lwid
c      integer*4 nd_next,jp,mm_dn,jlay
c      real*8 c4,c_dn,dwid,lwid
c     BD
c     BD
c
      if(nd_next .gt. nduct) then
         if(iidiag .ge. 1) then
c yt            print *,'Peak skipped (two in a row): ',jlay
         endif
         return
      endif
c
...
c: If previous duct less than lambda/5, delete it:
         nduct=nduct-1
         if(iidiag .ge. 1) then
c yt            print *,'Duct with width < lambda/5 deleted.'
         endif
      endif
c
      return
