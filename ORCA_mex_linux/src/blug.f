...
c: kt=4 means that cp2 is given 2nd, g instead of beta at end:
         ...
         bden=g0*h(klay) - delc
         if(bden .eq. 0.) then
cyt            print *,'blug layer ',j,'illegal: cp1,cp2,g gives ',
cyt     .         'a linear profile.'
            stop
         endif
         beta=delc**2/(2.*geo(1,1,klay)*bden) - 1.
      endif
      if(g0 .eq. 0. .or. rad .lt. 0.) then
cyt         print *,'illegal blug profile: j,g0,rad = ',j,g0,rad,
cyt     .      geo(1,1,klay),geo(2,1,klay),beta
         stop
      endif
c
      kdo=klay
...
ccc
      subroutine add_layer(k,kp1,ktot,zpt,cpt,geo,h,fexp)
c
      implicit none
      integer*4 k,kp1,ktot,j,j1,j2,ji
      real*8 zpt,cpt,geo(2,5,kp1),h(kp1),fexp(2,kp1),fac,hold
c
