...
cyt      print *,'nmode = ',nmode,(mode_phz(j,nmode),j=1,3)
c
      return
      end
ccc
...
c     BD
c     BD - phi_t, phi_b, dphi_t, dphi_b are regularly passed a complex*16
c      complex*8 phi_t,phi_b,dphi_t,dphi_b
      complex*16 phi_t,phi_b,dphi_t,dphi_b
c     BD
c     BD
c
...
c: Force layer phase difference to be zero when evanescent:
         ...
         phz_phi=phz_phi + phz_lay(j)
cyt      print *,nmode,j,iiev(1,j),iiev(2,j),phz_lay(j)/piex,phz_top/piex,
cyt     .   phz_bot/piex,(phz_top-phz_bl)/piex
         phz_bl=phz_bot
         if(iisol(j) .eq. 1) then
            call phi_phase(psilay(1,j),dpsilay(1,j),beti(1,1,j),
...
cyt      print *,'nmode = ',nmode,(mode_no(j,nmode),j=1,3),
cyt     .   phz_phi/piex,phz_psi/piex,phz_duct/piex
c
      return
      end
ccc
