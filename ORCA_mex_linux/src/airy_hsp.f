...
c: explicitly
cyt         print *,'Using user-defined Airy halfspace gradient ',
cyt     .      geolay(ii1,1) - geolay(ii2,1),geolay(ii1,4) - geolay(ii2,4)
         ii_xi_mv=0
         return
...
cyt         print *,'Error: Airy gradient direction ',
cyt     .      '[atan2(da,dc)] must be in [0,180] interval.'
         stop
      endif
      if(Aih_mag .lt. 0.0005) then
cyt         print *,'Error: Airy gradient magnitude must ',
cyt     .      'be > 0.0005 (recommended interval = [.001,.01]).'
         stop
      endif
      if(Aih_mag .gt. 0.5) then
cyt         print *,'Warning: Airy gradient should be ',
cyt     .      'less than 0.5 (recommended interval = [.001,.01]).'
      endif
...
cyt            if(iidiag .ge. 1) print *,
cyt     .         'BLM non-hspace: ',sngl(dreal(k)/kw0),
cyt     .         sngl(dimag(k)*8685.9),xi_re,xi_rat,sngl(f_hz)
...
cyt         if(iidiag .ge. 2) print *,'ki_eff = ',jm,
cyt     .      8.6859*log(abar)/rmin,sngl(8685.9*kim_max),abar
...
cc          endif
            if(iidiag .ge. 1) then
cyt               print *,'Airy contour done due to phimag weakness',
cyt     .            jm,8.6859*log(abar)/rmin,xi_re,xi_rat
            endif
         endif
         phiw_last=phiw_max
      else
