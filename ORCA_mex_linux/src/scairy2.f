...
c xx      print *,'ai_flip for re(zeta)<0: ',zeta2,ai2,aip2,bi2,bip2
         zfac=cdexp(-zeta2)
     ...
     .      1.e-12) iibad=1
      ...
      det2=det_pos
c xx   call wronsk_ch(ai2,aip2,bi2,bip2,det2)
c
      return
      end
c
...
c xx   ELSE IF (ATAN2(ZPR(2),ZPR(1)) .LE. TPIO3)  THEN
c xx   ELSE IF (abs(ATAN2(ZPR(2),ZPR(1))) .LE. TPIO3)  THEN
...
c ekw          IF (ZPR(2).GE.0.0D0)  THEN
   ...
   10 CONTINUE
c yt      WRITE (6,*) ' SERIES APPROXIMATION FAILED TO CONVERGE'
...
C   *********************************************************
      ...
      IF (ABS (Z) .LE. 3.5D0)  THEN
c ekw      CALL ABSERIES2 (Z, AI,AIP,BI,BIP)
          call airy_only_series (Z, AI,AIP,ZETA)
c ekw      IF (IRET.NE.0)  CALL FORMAIRY (AI,BI,AIP,BIP,Z,ZETA,0,IRET)
...
c xx   ELSE IF (abs(ATAN2(ZPR(2),ZPR(1))) .LE. TPIO3)  THEN
      ELSE IF (dimag(zp) .gt. -1.73205080756888d0*dreal(zp)) then
          IF (ABS(ZP).GT.7.9111D0)  THEN          
c ekw          CALL ABGAUSS2 (ZP, AI, AIP, BI, BIP, ZETA)
              call AGAUSS2 (ZP,AI,AIP,ZETA)
              IF (ICONJG.EQ.1)  THEN              
                  ZETA = CONJG (ZETA)
                  AI  = CONJG (AI)
          ...
          ELSE
              IF (ABS (ZP-(-0.9D0,2.8D0)) .LT. 4.97D0)  THEN
c ekw              CALL ASERIES2 (ZP, AI,AIP, ZETA)
              ...
              END IF
          END IF
c ekw      IF (IRET.LE.0)  CALL FORMAIRY (AI,BI,AIP,BIP,Z,ZETA,1,IRET)
...
c : Use connection formula: 
c : Ai(z)=exp(ipi/3)*Ai(z*exp(-i2pi/3)) + exp(-ipi/3)*z*exp(i2pi/3)
c ekw          ZETA  = Z * SQRT(Z) * ONEOOF
c ekw          E1  = EXP (ZETA-ZETA1)
c ekw          E2  = EXP (ZETA-ZETA2)
c ekw          AI  = ei13 * AI1*E1  + eim13 * AI2*E2
c ekw          AIP = eim13 * AIP1*E1 + ei13 * AIP2*E2
c ekw: Take out exp(zeta1) or exp(zeta2), whichever has smaller real part:
