...
c
     ...
     .         mzduct,pct_diff)
cyt            if(iidiag .ge. 2) print *,'Dup eigenvalues in different ',
cyt     .         'ducts: pct_diff = ',pct_diff
c
            if(pct_diff .lt. 1.e-2) then
               if(iidiag .ge. 2) then
cyt                  print *,'Duplicate mode found for duct: ',iduct,
cyt     .               kn(jm0),kn(nmode)
               endif
               ndup=ndup + 1
               return
            endif
cyt            if(iidiag .ge. 2) print *,'Close eigenvalues found: ',
cyt     .         kn(jm0),kn(nmode)
         endif
...
c: Inserts mode into correct place and moves other up:
      implicit none
      integer*4 nmode,kn_indx(nmode+1),nzsr,nzref(nmode),ncalc(nmode),
...
cxx   pct_diff=min(psum1,psum2)/max(1.e-200,psum)
      pct_diff=max(min(psum1,psum2)/max(1.e-30,psum),
