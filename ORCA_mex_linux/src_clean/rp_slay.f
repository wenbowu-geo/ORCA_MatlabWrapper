...
c
c xx   if(Rmat(1,4) .eq. zzero .or. Rmat(1,2) .eq. zzero) then
...
c xx1  delta1=q2*r1 - q1*r2
c xx1  delta2=q4*r3 - q3*r4
      delta1=-2.d0*r2
      delta2=2.d0*q4
...
c xx      a9_den=fac*gami1(1)*cdexp(-ze)
         a9_den=fac*gami1(1)
         Wmat(1)=a9_den*b8
         Wmat(2)=-a9_den*b7
c : Include separate exponential factor for Wpp and Wps:
         Wmat(5)=-ze
c xx      a9bar_den=fac*beti1(1)*cdexp(-zs)
         a9bar_den=fac*beti1(1)
         Wmat(3)=-a9bar_den*a7
         Wmat(4)=a9bar_den*a8
c : Include separate exponential factor for Wss and Wsp:
         Wmat(6)=-zs
c xx   print *,'prop 1',q1*Wmat(1)+r1sp*Wmat(2),cdexp(ze)*p(1)*
c xx  .   (e1p+e1m*Vmat(1,1))+cdexp(-ze)*u(1)*s2m*Vmat(1,2)
c xx   print *,'prop 2',q2*Wmat(1)+r2sp*Wmat(2),cdexp(ze)*q(1)*
c xx  .   (e2p+e2m*Vmat(1,1))+cdexp(-ze)*v(1)*s1m*Vmat(1,2)
c xx   print *,'prop 3',q3sp*Wmat(1)+r3*Wmat(2),-cdexp(ze)*u(1)*
c xx  .   (e2p+e2m*Vmat(1,1))+cdexp(-ze)*p(1)*s1m*Vmat(1,2)
c xx   print *,'prop 4',q4sp*Wmat(1)+r4*Wmat(2),-cdexp(ze)*v(1)*
c xx  .   (e1p+e1m*Vmat(1,1))+cdexp(-ze)*q(1)*s2m*Vmat(1,2)
c xx   print *,'prop 5',q3sp*Wmat(4)+r3*Wmat(3),cdexp(zs)*p(1)*
c xx  .   (s1p+s1m*Vmat(1,3))-cdexp(-zs)*u(1)*e2m*Vmat(1,4)
c xx   print *,'prop 6',q4sp*Wmat(4)+r4*Wmat(3),cdexp(zs)*q(1)*
c xx  .   (s2p+s2m*Vmat(1,3))-cdexp(-zs)*v(1)*e1m*Vmat(1,4)
c xx   print *,'prop 7',q1*Wmat(4)+r1sp*Wmat(3),cdexp(zs)*u(1)*
c xx  .   (s2p+s2m*Vmat(1,3))+cdexp(-zs)*p(1)*e1m*Vmat(1,4)
c xx   print *,'prop 8',q2*Wmat(4)+r2sp*Wmat(3),cdexp(zs)*v(1)*
c xx  .   (s1p+s1m*Vmat(1,3))+cdexp(-zs)*q(1)*e2m*Vmat(1,4)
      endif
c
