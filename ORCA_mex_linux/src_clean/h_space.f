...
c c       call clairy(xi,1,ai1,bi1,aip1,bip1,zzexp1)
c c    if(magsq(ai1x*exp(-zzexp1x)-ai1*exp(-zzexp1)) .gt. 1.d-16) then
c c       print *,'airy_only problem: ',ai1x*exp(zzexp1x),
c c   .      ai1*exp(zzexp1)
c c    endif
...
c c             xi_w=-2.d0*(xksq/w + eta*eta_w*xi)/etasq
