...
c : Take from rp_prop, but set r1=r2=q3=q4=0:
         ...
         wden=a7*b8 - b7*a8
c xx      fac=-2.d0*(p(1)*q(1) + u(1)*v(1))/wden
         ...
         Wmat(4)=a9bar_den*a8
         Wmat(6)=-zs
c xx   print *,'Wmat 1',q1*Wmat(1),cdexp(ze)*p(1)*
c xx  .   (e1p+e1m*Vmat(1,1))+cdexp(-ze)*u(1)*s2m*Vmat(1,2)
c xx   print *,'Wmat 2',q2*Wmat(1),cdexp(ze)*q(1)*
c xx  .   (e2p+e2m*Vmat(1,1))+cdexp(-ze)*v(1)*s1m*Vmat(1,2)
c xx   print *,'Wmat 3',r3*Wmat(2),-cdexp(ze)*u(1)*(e2p+e2m*
c xx  .   Vmat(1,1))+cdexp(-ze)*p(1)*s1m*Vmat(1,2)
c xx   print *,'Wmat 4',r4*Wmat(2),-cdexp(ze)*v(1)*(e1p+e1m*
c xx  .   Vmat(1,1))+cdexp(-ze)*q(1)*s2m*Vmat(1,2)
c xx   print *,'Wmat 5 ',q1*Wmat(4),cdexp(zs)*u(1)*(s2p + s2m*
c xx  .   Vmat(1,3)) + cdexp(-zs)*p(1)*e1m*Vmat(1,4)
c xx   print *,'Wmat 6 ',q2*Wmat(4),cdexp(zs)*v(1)*(s1p + s1m*
c xx  .   Vmat(1,3)) + cdexp(-zs)*q(1)*e2m*Vmat(1,4)
c xx   print *,'Wmat 7 ',r3*Wmat(3),cdexp(zs)*p(1)*(s1p + s1m*
c xx  .   Vmat(1,3)) - cdexp(-zs)*u(1)*e2m*Vmat(1,4)
c xx   print *,'Wmat 8 ',r4*Wmat(3),cdexp(zs)*q(1)*(s2p + s2m*
c xx  .   Vmat(1,3)) - cdexp(-zs)*v(1)*e1m*Vmat(1,4)
      endif
c
      do j=2,ndv
         if(iiz .eq. 1) then
