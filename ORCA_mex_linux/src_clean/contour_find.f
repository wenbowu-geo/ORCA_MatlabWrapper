...
c
10    continue
cyt      if(iimt .eq. 1) call mode_traj(k0,rr0,0)
      nm1=nmode + 1
      nc00=nctot
...
cyt               print *,'traj_sdp unsuccessful '
         ...
         if(dimag(k0) .gt. kim_min + dkim) then
            ...
            if(iiblm .ne. 0) then
               iidiff=0
cyt               if(iidiag .ge. 1) print *,'BLM found from contour_find'
            else
...
c: branch:
cyt                  print *,'Main branches different???  Tell EKW.'
                  iidiff=0
...
c
      ...
      ii_usek=0
c
cyt      print *,'Starting point from circle: ',dreal(klast)/kw0,
cyt     .   dimag(klast)*8685.9
10    continue
...
c
      ntry1=ntry1 + 1
      if(ntry1 .gt. 50) then
cyt         print *,'Failure in traj_sdp: ntry1 = ',ntry1
         iifail=1
         return
      endif
...
20    continue
      ntry2=ntry2 + 1
      if(ntry2 .gt. 4) then
cyt         print *,'Failure in traj_sdp: ntry2 = ',ntry2,
cyt     .      maglast,mag_spt,dreal(k_spt)/kw0,dimag(k_spt)*8685.9
cyt         print *,'Num: ',(ln_spt-lnlast)/(2.*delk),dln_spt,dlnlast
         iifail=1
         return
      endif
...
cyt         print *,'Lower mag of deriv: ',dreal(klast)/kw0,
cyt     .      dimag(klast)*8685.9
c: Magnitude of derivative decreased:
         ...
         dlnlast=dln_spt
         maglast=mag_spt
cyt         if(iimt .ne. 0) call mode_traj(k_spt,r1r2,0)
         goto 10
      else
c: If derivative increased, decrease delk:
cyt         if(iimt .ne. 0) call mode_traj(k_spt,r1r2,-1)
         delk=0.5d0*delk
         k_spt=klast + delk
         goto 20
      endif
...
c
cyt      print *,'Saddle pt: ',dreal(k_spt)/kw0,dimag(k_spt)*8685.9
      lnmag=dreal(ln_spt)
...
cyt      print *,'After traj_phase: ',dreal(k)/kw0,dimag(k)*8685.9
c
      return
      end
ccc
...
c
      if(ntry .gt. 10) then
cyt         print *,'ntry>10 in traj_phase: ,ntry,k,k/kw,iish = ',
cyt     .      ntry,k,k/kw,iish
cyt         print *,'k,klast,delk = ',k/kw,klast/kw,delk/kw,lnrrdes,
cyt     .      lnlast,dlnlast
cyt         print *,'xkbp,iish = ',xkbp(1,1)/kw,iish
cyt         print *,'num deriv = ',(r1r2(1,4)-lnlast)/(k-klast),dlnlast,
cyt     .      r1r2(2,4)
         if(ntry .gt. 11) then
            iifail=1
cc          call stop_run
...
c
      ...
      lnlast=lnr1r2
      dlnlast=dlnr1r2
cyt      if(iimt .ne. 0) call mode_traj(k,r1r2,0)
c
...
cyt      if(iimt .eq. 1) call mode_traj(k,r1r2,0)
c
c: Follow constant arg(R1R2) to |R1R2|=1 contour:
     ...
     .   rr_st,0.1d0,0.19635d0)
      if(iifail .ne. 0) then
cyt         print *,'Failure in traj_phase from contour_find2'
         iidiff=0
      endif
c
      return
