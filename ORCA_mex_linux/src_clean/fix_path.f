...
c: Move along phase contour to |R1R2|=1 contour:
     ...
     .      dlnlast,k0,rr0,.05d0,0.19635d0)
         if(iifail .eq. 1) then
cyt            print *,'Failure in traj_phase from fix_path: ',nmode,k0,rr0
cyt            call stop_run
            return
         endif
cyt         if(iidiag .ge. 2) print *,'traj_phase from fix_path: ',
cyt     .      k0,(rr0(j,4),j=1,2)
         ...
         if(branch .ne. (0.d0,0.d0)) then
            ...
            if(dreal(delk) .lt. 0.d0 .or. dimag(delk) .lt. 0.d0) then
               k0=kcut0
               call sheet_init(k0,1,iish,iish_ref)
cyt      print *,'Calling cut_cross2 after bad traj_phase: ',k0/kw0
               call r1r2_calc(k0,rr0,2,0)
...
cc       print *,'cut_cross: ',k0,(rr0(j,4),j=1,2)
         if(iidone .eq. 1) then
            nmode=nmode - 1
cyt            print *,'no crossing found '
            return
         elseif(iifail .eq. 1) then
            nmode=nmode - 1
cyt            print *,'Failure in cut_cross from fix_path: ',
cyt     .         nmode,k0,rr0
            call stop_run
            return
         endif
...
cyt         print *,'Failure in cut_cross'
         iifail=1
         return
...
cyt         if(kcub .eq. 0) print *,'kcub = 0 from cut_cross'
...
55       ntry=ntry+1
         ...
         if(ntry .gt. 8) then
            ...
            ntry2=ntry2 + 1
            if(ntry2 .gt. 20) then
cyt               print *,'Failure to pick up path in move_up_cut2: ',
cyt     .            k0/kw,kneg/kw
               iifail=1
               return
            endif
         ...
         kneg=kneg2
cyt         if(iidiag .ge. 2) print *,'Pos re(ln) done: ',k0/kw,
cyt     .      kneg/kw,rrneg(1,4),rrneg(2,4)
      endif
c
      kpos=k0
      ntry=0
10    kpos=dcmplx(dreal(kpos),dimag(kpos) + delki)
      ntry=ntry + 1
      if(ntry .gt. 50) then
cyt         print *,'Failure to find kpos in move_up_cut2: ',k0/kw,kpos/kw
         iifail=1
         return
      endif
...
20    k0=dcmplx(dreal(k0),.5d0*(dimag(kneg)+dimag(kpos)))
      ntry=ntry + 1
      if(ntry .gt. 20) then
cyt         print *,'Unsuccessful move_up_cut2: ',
cyt     .         kneg/kw,rrneg(2,4),kpos/kw,rrpos(2,4)
         iifail=1
         return
      endif
      call r1r2_calc(k0,rr0,2,0)
...
cxx  .   kpos/kw,kneg/kw,k0/kw,rr0(1,4),emagmax
            ...
            if(iidiag .ge. 2) then
cyt               print *,'emagmax OK, but derivatives wrong sign: ',
cyt     .            kneg/kw,rrneg(2,4),kpos/kw,rrpos(2,4)
            endif
         endif
      endif
