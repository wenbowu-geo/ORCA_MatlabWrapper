...
c zs: Take out exponential factor from A1fac and B1fac if necessary:
...
c xx         phix(jz)=A1fac + B1fac
c xx         dphix(jz)=inc*gami*(A1fac - B1fac)
         enddo
      else
...
c : Compute Airy function arguments at top and bottom of layer:
         ...
         enddo
c xx      call scairy3(nz,xi,xi0,ai,aip,bi,bip,zzexp,det,-aisoln)
...
c c          print *,'-eta = ',-eta,
c c   .         atan2(-dimag(eta),-dreal(eta))*180/acos(-1.)
...
c xx   print *,'bot: a1,b1 = ',a1*cdexp(zeta1),b1*cdexp(-zetb1),
c xx  .   iir1,iir2
         do jz=1,nz
c : EKW FIX:
            if(zmi(jz) .ne. 0.d0) then
c j3: check to see if xi(source) needs different solution for stability
...
c j3: needed values calculated already by src_recalc, so exit and proceed
                  goto 101
               endif
            endif
...
c xx         phix(jz)=A1fac*ai(jz) + B1fac*bi(jz)
c xx         dphix(jz)=-inc*eta*(A1fac*aip(jz) + B1fac*bip(jz))
101      enddo
      endif
99    continue
c
...
c
      ...
      if(iir1 .eq. 1) then
         ...
         ii=ii1
         if(iir1 .eq. 1) then
c c          print *,'A1 iso unstable at top & bot: ',rat1,rat2
...
c
      ...
      if(iir2 .eq. 1) then
         ...
         ii=ii1
         if(iir2 .eq. 1) then
c c          print *,'B1 iso unstable at top & bot: ',rat1,rat2
            if(rat1 .lt. rat2) then
               B1=B1x
               ii=ii2
            endif
...
c    complex*16 A1exp,B1exp,A1fac,B1fac,ph,dph
      real*8 Aplay(2),Aprefa,Aprefb,rat1,rat2
c
      detinv=1.d0/det
      ...
      A1=round_check(term1,term2,detinv,iir1,rat1)
c xx   A1=detinv*(bilay(2,ii2)*philay(ii2) + bilay(1,ii2)*dphfac)
      ii=ii2
      if(iir1 .eq. 1) then
         A1x=A1
         ...
         ii=ii1
         if(iir1 .eq. 1) then
c          print *,'A1 unstable at top & bot: ',rat1,rat2
            if(rat1 .lt. rat2) then
               A1=A1x
               ii=ii2
            endif
...
c
      ...
      B1=round_check(term1,term2,detinv,iir2,rat1)
c xx   B1=detinv*(-ailay(2,ii2)*philay(ii2) - ailay(1,ii2)*dphfac)
      ii=ii2
      if(iir2 .eq. 1) then
         B1x=B1
         dphfac2=inc*dphilay(ii1)/eta
         ...
         ii=ii1
         if(iir2 .eq. 1) then
c          print *,'B1 unstable at top & bot: ',rat1,rat2
            if(rat1 .lt. rat2) then
               B1=B1x
               ii=ii2
            endif
...
c
c c       do ii=1,2
c c          A1exp=Aprefa + (zeta1 - zetalay(ii))
c c          B1exp=Aprefb - (zetb1 - zetalay(ii))
c c          A1fac=A1*cdexp(A1exp)
c c          B1fac=B1*cdexp(B1exp)
c c          ph=A1fac*ailay(1,ii) + B1fac*bilay(1,ii)
c c          dph=-inc*eta*(A1fac*ailay(2,ii) + B1fac*bilay(2,ii))
c c    print *,'a1b1_airy: ii,phi = ',ii,ph,philay(ii)*dexp(Aplay(ii))
c c    print *,'   dphi = ',ii,dph,dphilay(ii)*dexp(Aplay(ii))
c c       enddo
c
      return
      end
...
c xx   if(ratio .gt. 1.d-18) then
      if(ratio .gt. 1.d-14) then
         ...
         iir=1
         round_check=(0.,0.)
c xx      print *,'Round to zero: ',term1,term2,numerator,deninv
      endif
c
      return
      end
