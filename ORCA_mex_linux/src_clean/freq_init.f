...
c: 2=upper) and wave type iips (1=p-wave,2=s-wave):
         ...
         if(iidiag .ge. 2) then
cyt            print *,'BP = ',ii,xkbp(ii,1),xkbp(ii,2)
cyt            print *,'k/kw = ',ii,xkrat(ii,1),xkrat(ii,2)
         endif
         inc=jflu(ii,3)
...
c: attenuation alpha (in dB/(m-kHz)).
      implicit none
      complex*16 xk,xksq
      real*8 w,c0,alpha,fexp,fac,w0,alpha_w
c: fac=2*pi*1000*20*log(e):
      data fac/5.457505415367364d+04/,w0/6.283185307179586d+03/
c
      if(c0 .ne. 0.) then
...
c: alpha_w=alpha * w0 (as before):
         alpha_w=alpha * ((w/w0)**fexp) * w0
         xk=dcmplx(w/c0,alpha_w/fac)
         xksq=xk*xk
      endif
c: Bottom line is: [w] * alpha/fac ==> [(w/w0)^fexp * w0] * alpha/fac
c
      return
      end
ccc
